...
1. **Database & Models**
   - `Product` table with:
     - `id`, `name`, `type_id`, `intensity`, `origin`, `image_url`, `price` (optional)
   - `Type` table (e.g., `whole beans`, `capsules`, `ground`) with associated image.
   - Seeder and factory to generate realistic mock data for products and types.

2. **CRUD Operations**
   - Admin can **Create, Read, Update, Delete** coffee products.
   - Forms should be labeled clearly and provide accessible error messages.

3. **Filtering**
   - Filter products by at least **two attributes** (e.g., intensity, origin).
